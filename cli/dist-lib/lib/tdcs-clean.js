 ...
 */
export function listCsvFiles(inputDir) {
    const results = [];
    function walk(dir) {
 ...
 */
export function readOneCsv(filePath) {
 ...
 */
export function cleanRawDf(rawRows, gantryFilterList, year, month, dateStart = 1, dateEnd = 31) {
    ...
    for (const row of rawRows) {
        ...
        if (dt.year !== year || dt.month !== month)
            continue;
        if (dt.day < dateStart || dt.day > dateEnd)
            continue;
        // 匝道篩選（O OR D）
        if (filterSet.size > 0) {
 ...
 */
export function buildHourlyAggregation(cleanedRows) {
    const map = new Map();
 ...
 */
export function mergeHourlyAccumulator(acc, part) {
    if (acc.length === 0)
        return part;
 ...
 * WeekIndex = Math.floor((Day - 1) / 7) + 1
 */
export function addWeekIndex(rows) {
    return rows.map(r => ({
        ...r,
...
function sortHourly(rows) {
    return rows.sort((a, b) => {
        if (a.Day !== b.Day)
            return a.Day - b.Day;
        if (a.Hour_0 !== b.Hour_0)
            return a.Hour_0 - b.Hour_0;
        // GantryID_O: string comparison（ASCII 排序，等同 Python）
        if (a.GantryID_O < b.GantryID_O)
            return -1;
        if (a.GantryID_O > b.GantryID_O)
            return 1;
...
export function toCsvString(rows) {
    ...
    const dataLines = rows.map(r => `${r.Year},${r.Month},${r.Day},${r.Weekday},${r.Hour_0},${r.GantryID_O},${r.VehicleType},${r.counts},${r.WeekIndex}`);
    // pandas to_csv 的尾行有 \r\n
 ...
 */
function saveOutputs(rows, outputDir, year, month) {
    ...
    fs.writeFileSync(path.join(monthlyDir, `M06A_${tag}_hourly_counts_all.csv`), toCsvString(rows), { encoding: 'utf8' });
    ...
    for (const [weekIdx, wRows] of [...weekMap.entries()].sort((a, b) => a[0] - b[0])) {
        fs.writeFileSync(path.join(weeklyDir, `M06A_${tag}_week${weekIdx}_hourly_counts.csv`), toCsvString(wRows), { encoding: 'utf8' });
    }
    ...
    for (const [day, dRows] of [...dayMap.entries()].sort((a, b) => a[0] - b[0])) {
        ...
        fs.writeFileSync(path.join(dailyDir, `M06A_${tag}_day${dd}_hourly_counts.csv`), toCsvString(dRows), { encoding: 'utf8' });
    }
}
