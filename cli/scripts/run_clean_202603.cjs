#!/usr/bin/env node
/**
 * M5 Baseline 對齊 Runner（CJS，直接 require dist-lib）
 * 執行：node scripts/run_clean_202603.cjs
 */
'use strict';
...
// Import compiled CJS lib
const { runCleaning } = require('../dist-lib/lib/tdcs-clean');
...
console.log(`TS 清洗完成: ${result.file_count} 檔 | 掃描 ${result.scanned_rows.toLocaleString()} | 清洗 ${result.cleaned_rows.toLocaleString()} | 彙總 ${result.hourly_rows.toLocaleString()} | ${elapsed}s`);
...
for (const r of diffs) {
  const icon = r.status === 'PASS' ? 'OK' : r.status === 'FAIL' ? 'FAIL' : r.status;
  reportLines.push(`| \`${r.key}\` | ${icon} | ${r.pyLines} | ${r.tsLines} | \`${String(r.pyMd5).slice(0,8)}\` | \`${String(r.tsMd5).slice(0,8)}\` |`);
}
if (fail > 0) {
