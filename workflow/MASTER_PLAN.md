...
### ⚠️ PLAN_E10 線性走 pivot（2026-06-04）

PLAN_E9 close 後 User 確認兩件大事：
1. **期末後砍 AWS、純本地 npm v3.0**（見 §11 Changelog / `project_post_demo_public_repo` memory）→ Athena subcommand / npm publish / GitHub release 全部 post-demo 重新設計、現走 = 浪費功
2. **Demo 6/10 在即**（PLAN_E9 close 後僅剩 6 天）→ 期末三件套必須先做、原 E13 拉前到 E10

**故重排**：
- ~~PLAN_E10：Athena 整合 + query subcommand~~ → demo 後重新評估（可能整段砍）
- ~~PLAN_E11：UX 打磨 + npm publish~~ → demo 後重新評估（npm publish 改 v3.0 純本地版重做）
- ~~PLAN_E12：端到端 demo + integration test~~ → 3 cases 縮成 demo 本身 1 case（麻豆段已驗）+ 影片內展示
- **PLAN_E10 = 原 PLAN_E13 期末三件套**（書面 PDF + PPT + YouTube demo 影片）

下方 ~~刪除線~~ 段保留作為歷史脈絡、不再執行。

---

### ~~PLAN_E10 ‧ Athena 整合 + query subcommand~~（2026-06-04 砍、可能 demo 後 reset）
> 原派 `sonnet_worker`、Lead 審。
...
**砍因**：post-demo 砍 AWS、Athena 也砍、CLI `query` subcommand 改 DuckDB 本地查 Parquet（v3.0 設計）。Demo 用 AWS Console 直查 Athena 證明清洗結果（demo_13/demo_16 已截）已夠 narrative。

### ~~PLAN_E11 ‧ UX 打磨 + npm publish~~（2026-06-04 砍、demo 後 v3.0 重做）
> 原派 `sonnet_worker` 為主、Lead 設計品牌。
...
**砍因**：post-demo 砍 AWS = backend 換 = wizard 流程重設計 = UX 打磨在現版做會被 v3.0 推翻。npm publish 改 demo 後做、避免 v2.0 釋出又馬上被 v3.0 取代造成 user confusion。

### ~~PLAN_E12 ‧ 端到端 demo + integration test~~（2026-06-04 縮、併入 PLAN_E10 影片）
> 原 Lead + User 跑、無 worker。

**原任務**：3 個 demo cases 跑通 + integration test + UX 流暢驗證

**縮因**：3 cases 在 6 天內跑完無時間做書面 + PPT + 影片三件套。麻豆段已驗（事件 #11 / 事件 #14）、demo 影片只展示麻豆段 + 截圖 evidence chain（demo_15→15b→16）足夠。Integration test 改 post-demo 隨 v3.0 重寫一次。

### PLAN_E10 ‧ 期末三件套（書面 PDF + PPT + YouTube demo 影片）
> Lead 主寫、User 拍板 + 跑 AI Studio + 錄影片。可能派 `opus_worker` 寫長文段。

**任務**：
- M0: 修 spec typo 對齊（cleaned_v2 → cleaned_v2_skeleton / tdcs_dl_wg → tdcs-dl-wg、Lead 一次 grep 修）
- M1: Demo 素材補缺（demo_08 cleaning stage / demo_09 done view / demo_12 Athena workgroup、3 張、User 自跑 CLI 補拍）
- M2: 書面報告 .md 撰寫（Lead 從實驗紀錄 + MASTER_PLAN 抽 concept design highlights、含 8 大重點）
- M3: 書面 → PDF（Pandoc xelatex、Microsoft JhengHei + Times New Roman）
- M4: Presentation brief 整理（給 AI Studio 用、per workflow §11）
- M5: PPT 製作（User 在 AI Studio 跑 brief → HTML 動畫簡報、per `feedback_presentation_method`）
- M6: 影片 narration script（Lead 寫、中文口語 + 知識點兩段式、per `feedback_qa_style`）
- M7: 影片錄製 + 上傳 YouTube（User 自跑、deadline 2026-06-09）

**Concept design highlights**（M2 書面 + M5 PPT + M6 影片串成同一 narrative）：
1. 重複下載判斷 — S3 已存在則跳過（避免他人浪費上傳）
2. SQS broker pattern — API GW 30s vs Lambda 8 min timeout 鴻溝、producer-consumer 解
3. F-H4 git SHA tag versioning — :latest 鎖死無法回退、SHA tag immutable 解
4. F-H5 schema type drift bug + fix — JS Number 推 Float64 → Parquet DOUBLE → Glue int mismatch、demo_15→15b→16 三連 evidence chain
5. Lambda > Glue ETL trade-off — 9 GiB / < 5 min / 簡單 transform、Lambda sweet spot
6. E6 client-side relay winner — 三跳板實驗（E3 EC2、E4 Cloudflare Worker、E6 client-side）的科學選擇
7. Wizard 三選項 output — 本機 / S3 / 混合、user 自選資料落點
8. §0.3 4-step compromise mechanism — baseline pivot byte-md5 → structural 三證、永久債避免

**驗收**：三件套交件 + 課程展示通過（6/10）+ 影片上傳 YouTube（6/9）
...
| **sync copy lib drift**（PLAN_E9 M1 D1 設計、實驗紀錄9 事件 #12 紀錄）| Lambda `infra/lambda/src/lib/tdcs-clean.ts` 是 cli/ `src/lib/tdcs-clean.ts` 的 sync copy（diff 只差 3 行同步 header）、PLAN_E9 baseline 對齊靠此公理（=「lambda 邏輯 = cli 邏輯 = Python 原版」傳遞性）。**風險**：未來如 cli/ts-clean 改邏輯、忘了同步 lambda/、structural 三證會通（rows + Athena 對得上）但實際結果靜默 mismatch。**緩解**：(1) PLAN_E11 啟動前 gate 加「跑 byte-level 對齊」一次性實證（cli ts on 744 raw → reference cleaned.csv → Lambda Parquet → polars writeCSV → diff、~30-50 min）;(2) 未來 cli/ts-clean 任何 PR 必同步 lambda/ts-clean、加 CI grep 或 pre-commit hook（PLAN_E11 評估）;(3) 評估抽 monorepo shared package（PLAN_E11 最徹底解、但 scope 大）|
| **runPull 孤兒 temp 檔**（2026-06-04 opus_worker_2 期中發現、本 session 已實際撞到 3.4 GB 孤兒）| `cli/src/commands/pull.ts` 的 `runPull` 清理邏輯只在 success path 跑、Ctrl+C 中斷 / 出錯路徑無 cleanup → 本機 temp 留下 GB 級孤兒檔。**緩解**：demo 後 v3.0 規劃做 `try/finally` + `SIGINT` handler 統一清理、與 streaming pull 設計一起做（PLAN_E10 §6 v3.0 設計章寫入、不寫進 §3 highlights）。**現階段不擋 demo**（user 自己手動清、opus_worker_2 期中已示範刪法）|
| **wizard step 4「本機儲存」UI 與 code 不一致**（2026-06-04 opus_worker_2 期中發現）| `cli/src/wizard/Output.tsx` 有「本機儲存→./tdcs-output/」選項 UI、但 `runClean` 端**未實作** GetObject cleaned.parquet → 寫本地的邏輯、本機磁碟實查無 ./tdcs-output/。**緩解**：PLAN_E10 §3.7 走概念設計寫（不謊稱本機有檔、§0.3 紅線 #1）、實作補 demo 後 v3.0 純本地版（DuckDB 取代 Athena 後本機本來就是 first-class path）。**現階段不擋 demo**（cloud path 已驗 22 GB → 14116 rows、是核心、本機輸出是便利不是核心）|
